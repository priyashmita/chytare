    ...
    <AdminLayout>
      <div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "32px", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "28px", fontWeight: 400, color: "#1B4D3E" }}>Suppliers</h1>
            <p style={{ fontFamily: SANS, fontSize: "13px", color: "rgba(27,77,62,0.5)", marginTop: "4px" }}>{suppliers.length} suppliers in system</p>
          </div>
          <Link to="/admin/suppliers/new" className="btn-luxury btn-luxury-primary" style={{ display: "flex", alignItems: "center", gap: "8px", whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", gap: "12px", marginBottom: "24px", flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: "1", minWidth: "200px" }}>
            <Search style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", width: 15, height: 15, color: "rgba(27,77,62,0.3)" }} />
            <Input
              value={search}
          ...
          <select
            ...
            style={{ fontFamily: SANS, fontSize: "13px", padding: "8px 12px", border: "1px solid rgba(218,203,160,0.5)", background: "white", color: "#1B4D3E", minWidth: "180px" }}
          >
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {[...Array(5)].map((_, i) => <div key={i} style={{ height: "64px", background: "rgba(218,203,160,0.1)", animation: "pulse 1.5s infinite" }} />)}
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "64px 0", background: "white", border: "1px solid rgba(218,203,160,0.3)" }}>
            <p style={{ fontFamily: SANS, fontSize: "14px", color: "rgba(27,77,62,0.5)", marginBottom: "16px" }}>No suppliers found</p>
            <Link to="/admin/suppliers/new" className="btn-luxury btn-luxury-secondary">Add First Supplier</Link>
          </div>
        ) : (
          <div style={{ background: "white", border: "1px solid rgba(218,203,160,0.3)", overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {filtered.map((s, i) => (
                  <tr key={s.id} style={{ borderTop: "1px solid rgba(218,203,160,0.15)", background: i % 2 === 0 ? "white" : "rgba(255,255,240,0.5)" }}>
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{ fontFamily: SANS, fontSize: "12px", letterSpacing: "0.05em", color: "rgba(27,77,62,0.5)", fontWeight: 500 }}>{s.supplier_code}</span>
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{ fontFamily: SANS, fontSize: "12px", color: "rgba(27,77,62,0.7)", background: "rgba(218,203,160,0.15)", padding: "3px 8px" }}>{s.supplier_type}</span>
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{ fontFamily: SANS, fontSize: "13px", color: "rgba(27,77,62,0.7)" }}>{s.lead_time_days ? `${s.lead_time_days} days` : "—"}</span>
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{ ...STATUS_STYLE[s.status] || STATUS_STYLE.active, fontFamily: SANS, fontSize: "11px", letterSpacing: "0.08em", textTransform: "uppercase", padding: "3px 8px", fontWeight: 500 }}>
                        {s.status || "active"}
                      </span>
                    </td>
                    <td style={{ padding: "14px 16px" }}>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent className="bg-[#FFFFF0]">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>Deactivate {s.supplier_name}? They will remain in the system but cannot be linked to new purchases.</AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
