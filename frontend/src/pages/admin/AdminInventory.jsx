              ...
              <table className="w-full">
                <thead className="bg-[#1B4D3E]/5">
                  <tr>
                    ...
                    <th className="text-left p-4 text-xs uppercase tracking-wider text-[#1B4D3E]/60">Stock</th>
                    <th className="text-left p-4 text-xs uppercase tracking-wider text-[#1B4D3E]/60">Price</th>
                    <th className="text-left p-4 text-xs uppercase tracking-wider text-[#1B4D3E]/60">Sold</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#DACBA0]/20">
                  {inventory.map((item) => (
                    <tr key={item.id} className={`hover:bg-[#FFFFF0] ${item.low_stock ? "bg-[#C08081]/5" : ""}`}>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          {item.low_stock && (
                            <AlertTriangle className="w-4 h-4 text-[#C08081]" />
                          )}
                          <div>
                      ...
                      <td className="p-4">
                        <span className={`text-sm ${item.low_stock ? "text-[#C08081] font-medium" : "text-[#1B4D3E]"}`}>
                          {item.stock_quantity}
                        </span>
                      </td>
