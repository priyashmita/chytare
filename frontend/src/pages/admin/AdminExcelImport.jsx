  ...
  const handleImport = async () => {
    ...
    try {
      const payload = validRows.map(row => {
        const clean = { ...row };
        delete clean.__rowNum; delete clean.__errors; delete clean.__valid;
        return clean;
      });
      const res = await axios.post(`${API}${IMPORT_ENDPOINTS[module.key]}`, { rows: payload });
      setResult(res.data);
      setPreview(null);
      setFile(null);
  ...
  return (
    <div style={{ background: "white", border: "1px solid rgba(218,203,160,0.3)", padding: "24px", marginBottom: "16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
        <div>
          <h3 style={{ fontFamily: SERIF, fontSize: "18px", fontWeight: 400, color: "#1B4D3E" }}>{module.label}</h3>
          <p style={{ fontFamily: SANS, fontSize: "12px", color: "rgba(27,77,62,0.5)", marginTop: "4px" }}>{module.description}</p>
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          {!module.exportOnly && (
            <button onClick={() => downloadTemplate(module)} style={{ display: "flex", alignItems: "center", gap: "6px", fontFamily: SANS, fontSize: "12px", padding: "8px 14px", border: "1px solid rgba(218,203,160,0.5)", background: "white", color: "#1B4D3E", cursor: "pointer" }}>
              <Download style={{ width: 13, height: 13 }} /> Download Template
            </button>
          )}
          <button onClick={() => downloadExport(module)} style={{ display: "flex", alignItems: "center", gap: "6px", fontFamily: SANS, fontSize: "12px", padding: "8px 14px", border: "1px solid rgba(27,77,62,0.2)", background: "rgba(27,77,62,0.04)", color: "#1B4D3E", cursor: "pointer" }}>
      ...
      {!module.exportOnly && module.required.length > 0 && (
        <p style={{ fontFamily: SANS, fontSize: "11px", color: "rgba(27,77,62,0.4)", marginBottom: "16px" }}>
          Required columns: {module.required.map(r => <span key={r} style={{ background: "rgba(27,77,62,0.06)", padding: "1px 6px", marginRight: "4px" }}>{r}</span>)}
        </p>
      )}
      ...
      {!module.exportOnly && (
        <>
          ...
          {preview && (
            <div style={{ marginTop: "16px" }}>
              <div style={{ display: "flex", gap: "16px", marginBottom: "12px", alignItems: "center" }}>
                <span style={{ fontFamily: SANS, fontSize: "12px", color: "#2d6e2d" }}>✓ {validCount} valid rows</span>
                {errorCount > 0 && <span style={{ fontFamily: SANS, fontSize: "12px", color: "#C08081" }}>✗ {errorCount} rows with errors</span>}
              </div>
              <div style={{ overflowX: "auto", maxHeight: "200px", border: "1px solid rgba(218,203,160,0.2)" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead style={{ background: "rgba(27,77,62,0.04)", position: "sticky", top: 0 }}>
                    <tr>
                      <th style={{ padding: "6px 10px", fontFamily: SANS, fontSize: "10px", letterSpacing: "0.08em", color: "rgba(27,77,62,0.4)", textAlign: "left" }}>Row</th>
                      {preview.headers.slice(0, 6).map(h => (
                        <th key={h} style={{ padding: "6px 10px", fontFamily: SANS, fontSize: "10px", letterSpacing: "0.08em", color: "rgba(27,77,62,0.4)", textAlign: "left" }}>{h}</th>
                      ))}
                      <th style={{ padding: "6px 10px", fontFamily: SANS, fontSize: "10px", color: "rgba(27,77,62,0.4)" }}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.rows.slice(0, 20).map((row, i) => (
                      <tr key={i} style={{ borderTop: "1px solid rgba(218,203,160,0.1)", background: row.__valid ? "white" : "rgba(192,128,129,0.04)" }}>
                        <td style={{ padding: "6px 10px", fontFamily: SANS, fontSize: "12px", color: "rgba(27,77,62,0.4)" }}>{row.__rowNum}</td>
                        {preview.headers.slice(0, 6).map(h => (
                          <td key={h} style={{ padding: "6px 10px", fontFamily: SANS, fontSize: "12px", color: "#1B4D3E", maxWidth: "120px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row[h] || "—"}</td>
                        ))}
                        <td style={{ padding: "6px 10px" }}>
                          {row.__valid
                            ? <CheckCircle style={{ width: 14, height: 14, color: "#2d6e2d" }} />
                            : <span style={{ fontFamily: SANS, fontSize: "10px", color: "#C08081" }}>{row.__errors[0]}</span>
                          }
                        </td>
                      </tr>
                    ))}
              ...
              <div style={{ display: "flex", gap: "10px", marginTop: "12px" }}>
                <button onClick={handleImport} disabled={importing || validCount === 0}
                  style={{ display: "flex", alignItems: "center", gap: "6px", fontFamily: SANS, fontSize: "12px", padding: "10px 20px", background: "#1B4D3E", color: "#FFFFF0", border: "none", cursor: importing ? "not-allowed" : "pointer", opacity: importing ? 0.5 : 1 }}>
                  <Upload style={{ width: 13, height: 13 }} />
                  {importing ? "Importing..." : `Import ${validCount} rows`}
                </button>
                <button onClick={() => { setPreview(null); setFile(null); }}
                  style={{ fontFamily: SANS, fontSize: "12px", padding: "10px 16px", background: "white", border: "1px solid rgba(218,203,160,0.5)", color: "#1B4D3E", cursor: "pointer" }}>
                  Cancel
                </button>
              </div>
            </div>
          ...
          {/* Result */}
          {result && (
            <div style={{ marginTop: "12px", background: "rgba(100,160,100,0.06)", border: "1px solid rgba(100,160,100,0.3)", padding: "12px 16px" }}>
              <p style={{ fontFamily: SANS, fontSize: "13px", color: "#2d6e2d", fontWeight: 500 }}>
                ✓ Import complete — {result.created} created, {result.skipped || 0} skipped, {result.errors || 0} errors
              </p>
            </div>
          )}
        </>
  ...
  <AdminLayout>
    <div>
      <div style={{ marginBottom: "32px" }}>
        <h1 style={{ fontFamily: SERIF, fontSize: "28px", fontWeight: 400, color: "#1B4D3E" }}>Excel Import / Export</h1>
      ...
      <div style={{ background: "rgba(27,77,62,0.04)", border: "1px solid rgba(218,203,160,0.3)", padding: "16px 20px", marginBottom: "24px" }}>
        <p style={{ fontFamily: SANS, fontSize: "13px", color: "#1B4D3E", fontWeight: 500, marginBottom: "4px" }}>How to import:</p>
        <p style={{ fontFamily: SANS, fontSize: "12px", color: "rgba(27,77,62,0.6)", lineHeight: 1.7 }}>
          1. Download the template CSV for that module<br/>
          2. Fill in your data (keep column names exactly as shown)<br/>
          3. Upload the filled CSV — you'll see a preview with validation<br/>
