      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "24px", flexWrap: "wrap", gap: "16px" }}>
          <div>
            ...
            <p style={{ fontFamily: SANS, fontSize: "13px", color: "rgba(27,77,62,0.5)", marginTop: "4px" }}>{orders.length} orders · ₹{totalRevenue.toLocaleString("en-IN")} collected</p>
          </div>
          <button onClick={() => navigate("/admin/orders/new")} className="btn-luxury btn-luxury-primary" style={{ display: "flex", alignItems: "center", gap: "8px", whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "24px", flexWrap: "wrap" }}>
          {Object.entries(counts).map(([status, count]) => (
            <button key={status} onClick={() => setFilterOrderStatus(filterOrderStatus === status ? "" : status)}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {filtered.map((o, i) => {
                  ...
                  return (
                    <tr key={o.id} style={{ borderTop: "1px solid rgba(218,203,160,0.15)", background: i % 2 === 0 ? "white" : "rgba(255,255,240,0.4)", cursor: "pointer" }}
