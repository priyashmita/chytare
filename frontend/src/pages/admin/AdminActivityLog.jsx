    ...
    <AdminLayout>
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <button onClick={fetchLogs} className="btn-luxury btn-luxury-secondary flex items-center gap-2 w-fit">
        ...
        {loading ? (
          <div className="space-y-2">{[...Array(8)].map((_, i) => <div key={i} className="h-14 bg-[#DACBA0]/10 animate-pulse" />)}</div>
        ) : (
          <div className="bg-white border border-[#DACBA0]/30 overflow-hidden">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#DACBA0]/10">
                {filtered.map((log) => {
                  const meta = ACTION_LABELS[log.action] || { label: log.action, color: "text-gray-600 bg-gray-50" };
                  return (
                    <tr key={log.id} className="hover:bg-[#FFFFF0]">
                      ...
                      <td className="p-4 text-xs text-[#1B4D3E]/60">
                        ...
                        {log.target_id && <span className="text-[#1B4D3E]/30 ml-1 font-mono">{log.target_id.slice(0, 8)}…</span>}
                      </td>
