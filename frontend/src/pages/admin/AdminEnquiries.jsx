...
const STATUSES = ["new", "contacted", "negotiating", "converted", "closed"];
...
const AdminEnquiries = () => {
  ...
  const handleBulkStatus = async () => {
    ...
    setBulkLoading(true);
    try {
      const res = await axios.post(`${API}/admin/enquiries/bulk-status`, { ids: [...selected], status: bulkStatus }, { headers: AUTH() });
      toast.success(`${res.data.updated} enquiries updated`);
      fetchEnquiries();
  ...
  return (
    <div data-testid="admin-enquiries">
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "24px", flexWrap: "wrap", gap: "16px" }}>
        <div>
          <h1 style={{ fontFamily: SERIF, fontSize: "28px", color: "#1B4D3E" }}>Enquiries</h1>
          <p style={{ fontFamily: SANS, fontSize: "13px", color: "rgba(27,77,62,0.5)" }}>{enquiries.length} total enquiries</p>
        </div>
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <ExportImportBar module="enquiries" filters={{ status: filterStatus, enquiry_source: filterSource, search }} />
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "24px", flexWrap: "wrap" }}>
        {Object.entries(counts).map(([status, count]) => (
          <button key={status} onClick={() => setFilterStatus(filterStatus === status ? "" : status)}
            style={{ ...STATUS_STYLE[status], fontFamily: SANS, fontSize: "12px", padding: "6px 14px", letterSpacing: "0.05em", cursor: "pointer", border: filterStatus === status ? `1px solid ${STATUS_STYLE[status]?.color}` : "1px solid transparent" }}>
            {count} {status}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "24px", flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: "200px" }}>
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by code, customer, product..." style={{ paddingLeft: "30px", fontFamily: SANS, fontSize: "13px" }} />
        </div>
        ...
        <select value={filterProduct} onChange={(e) => setFilterProduct(e.target.value)} style={{ fontFamily: SANS, fontSize: "13px", padding: "8px 12px", border: "1px solid rgba(218,203,160,0.5)", background: "white", color: "#1B4D3E", minWidth: "180px" }}>
          ...
          {meta.products.map(p => <option key={p.id} value={p.id}>{p.product_name}</option>)}
        </select>
      </div>
      ...
      {selected.size > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 16px", background: "rgba(27,77,62,0.06)", border: "1px solid rgba(27,77,62,0.15)", marginBottom: "12px", flexWrap: "wrap" }}>
          ...
          <select value={bulkStatus} onChange={e => setBulkStatus(e.target.value)} style={{ fontFamily: SANS, fontSize: "12px", padding: "5px 10px", border: "1px solid rgba(218,203,160,0.5)", background: "white", color: "#1B4D3E" }}>
            <option value="">Set status...</option>
            {STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <button onClick={handleBulkStatus} disabled={bulkLoading || !bulkStatus} className="btn-luxury btn-luxury-secondary" style={{ fontSize: "12px", padding: "6px 14px" }}>Apply</button>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead style={{ background: "rgba(27,77,62,0.04)" }}>
              <tr>
                ...
                {["Code", "Customer", "Product", "Source", "Status", "Date", ""].map(h => (
            ...
            <tbody>
              {filtered.map((e, i) => {
                ...
                const sourceBg = SOURCE_STYLE[e.enquiry_source] || SOURCE_STYLE.other;
                const isSelected = selected.has(e.id);
                return (
                  <tr key={e.id} style={{ borderTop: "1px solid rgba(218,203,160,0.15)", background: isSelected ? "rgba(27,77,62,0.03)" : i % 2 === 0 ? "white" : "rgba(255,255,240,0.4)", cursor: "pointer" }}
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ fontFamily: SANS, fontSize: "12px", letterSpacing: "0.05em", color: "rgba(27,77,62,0.5)", fontWeight: 600 }}>{e.enquiry_code || e.id?.slice(0, 8)}</span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <p style={{ fontFamily: SANS, fontSize: "13px", color: "#1B4D3E", fontWeight: 500 }}>{e.customer_name}</p>
                      {e.customer_email && <p style={{ fontFamily: SANS, fontSize: "11px", color: "rgba(27,77,62,0.4)", marginTop: "2px" }}>{e.customer_email}</p>}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ fontFamily: SANS, fontSize: "13px", color: "rgba(27,77,62,0.7)" }}>{e.product_name || "—"}</span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ fontFamily: SANS, fontSize: "11px", background: sourceBg, padding: "3px 8px", textTransform: "capitalize", color: "rgba(27,77,62,0.7)" }}>{e.enquiry_source || "website"}</span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ ...statusStyle, fontFamily: SANS, fontSize: "11px", letterSpacing: "0.06em", textTransform: "uppercase", padding: "3px 8px", fontWeight: 500 }}>{e.status}</span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                    ...
                    <td style={{ padding: "12px 16px" }} onClick={ev => ev.stopPropagation()}>
                      <div style={{ display: "flex", alignItems: "center", gap: "2px" }}>
                        <button onClick={() => navigate(`/admin/enquiries/${e.id}`)} style={{ padding: "6px", color: "rgba(27,77,62,0.5)", background: "none", border: "none", cursor: "pointer" }}>
                          <Eye style={{ width: 15, height: 15 }} />
                        </button>
                        <AuditHistoryModal entityType="enquiry" entityId={e.id} entityName={e.customer_name} />
                      </div>
                    </td>
                  </tr>
                );
